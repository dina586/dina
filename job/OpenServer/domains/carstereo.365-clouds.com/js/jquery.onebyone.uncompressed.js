...
;(function($) {	
    ...
    $.fn.oneByOne = function(options) {  
		...
		// disable image hover dragging *CUSTOM*
		_this.find(".oneByOne_item img").each(function() {									
		...
		function tweenTo(n){ 
			...
			/* CUSTOM STARTS */
			var __n = parseFloat(n)+1;
			var imgSrc = $(".oneByOne_item#slide-"+((n>=10)?"":"0")+__n).data("bg");
			if( typeof imgSrc != "undefined" ) {
				$(".slug-pattern").prepend("<div class='background-image' style='background-image:url("+imgSrc+");background-scroll:scroll;z-index:1' id='"+__n+"'></div>");
				$(".slug-pattern .background-image").each(function() { if( $(this).attr("ID") != __n ) $(this).fadeOut(1000,function(){$(this).remove();}); });
			}
