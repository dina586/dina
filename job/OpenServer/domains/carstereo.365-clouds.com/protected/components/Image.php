...
class Image extends CApplicationComponent {
	...
	public function resize($width, $height, $savePath, $imageName, $type = 'auto', $ext = false) {
		$imageName = $this->normalizeName($imageName);
	
		if(!$ext) {
			if($this->convertToJpg)
				$ext = 'jpg';
			else
				$ext = Yii::app()->cFile->set($this)->getExtension();
		}
	 ...
	 * @param int $height
	 */
	/*protected function getImageGeometry($width, $height) {
	 ...
	 * @return string
	 */
	private function normalizeName($imageName) {
