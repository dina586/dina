...
$(document).on('mouseenter', '#j-avatar_controls', function() {
	...
	$('#j-avatar_links').css({'height': '0px'}).show().animate({height: "36px"}, 300);
})
...
/**
 * Удаление аватара при нажатии
 */
$(document).on('click', '#a-avatar_delete a', function() {
	var url = $(this).attr('data-href');
	$.ajax({
		url: url,
		type: 'GET',
		beforeSend: function() {
			$('#j-avatar_controls').append('<div class = "b-profile_loader j-profile_loader"></div>');
		},
		success: function(data) {
			var json = $.parseJSON(data);
			
			if(json.status == 'ok') {
				$('#a-avatar_delete, #j-avatar_img > *').remove();
				$('#j-avatar_img').html(json.message);
				$('#d-profile_error').hide();
			} else {
				$('#d-profile_error p').html(json.message);
				$('#d-profile_error').removeClass('l-hidden').show();
				
			}
		},
		error: function(jqXHR, textStatus, errorThrown) {
			$('#d-profile_error').show().html(jqXHR);
		},
		complete: function() {
			$('.j-profile_loader').remove();
		}
	})
	return false;
})
