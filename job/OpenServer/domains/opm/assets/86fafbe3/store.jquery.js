		...
		$.ajax({
			...
			success: function(data){
				...
				if(item.number == 0) {
					...
					$('<p>Your cart is empty!</p>').appendTo('.j-shopping_cart');
				} else {
	...
	$(document).on('change', '.d-cart_table .j-cart_number', function() {
		...
		$.ajax({
		  ...
		  success: function(data){
			...
			$('.d-cart_table tr:eq('+index+') .j-sum_price').text(whiteSpace(sum_price.toFixed(2)));
			var item = $.parseJSON(data);
			$('.d-cart_total').text(item.number);
			var total = parseFloat(item.total_cost).toFixed(2);
			$('.d-cart_cost').text(whiteSpace(total));
		  }
		});
	})
...
$(document).on('change', '.j-cart_international input', function() {
	if($(this).is(':checked')) {
		$('.j-cart_us').hide();
		$('.j-cart_internation select').val('');
		$('.j-cart_internation').show();
	}
	else {
		$('.j-cart_us').show();
		$('.j-cart_us select').val('');
		$('.j-cart_internation').hide();
	}
})
$(document).on('change', '.j-cart_internation select, .j-cart_us select', function() {
	var val = $(this).val();
	$('.j-shipping_price').text(val);
})
