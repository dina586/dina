...
$image = Helper::getCover($data->id, get_class($data), Yii::app()->createUrl('store/product/view', array("url"=>$data->url)), 'thumbnail', '<img src = "/images/no-product.png"/>');
...
/*if($data->share_price != '' && $data->share_price != 0) {
	...
	$cost = '<span class = "b-store_preview_current_price">$ <span class = "j-item_price">'.StoreHelper::viewPrice($data->price).'</span></span> ';
//}
...
<div class = "b-store_preview l-inline_block<?=$jClass;?><?=$class;?> j-item_height_container">
	...
	<section class = "b-store_preview_cart">
		<a class = "a-add_to_cart btn" href = "<?=Yii::app()->createUrl('store/cart/addToCart',array('id'=>$data->id));?>"><?=Yii::t('shop', 'add to cart')?></a>
	</section>
</div>
