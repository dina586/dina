...
class StoreHelper {
	...
	public static function getPaymentData($data){
		if($data['international']==1)
			$data['shipping'] = $data['int_shipping'];
		else
			$data['shipping'] = $data['us_shipping'];
		$p['total_cost'] = Yii::app()->shoppingCart->getCost() + $data['shipping'];
		$p['description'] = Yii::app()->getModule('invoice')->type[1];
		$p['relay_url'] = Yii::app()->createAbsoluteUrl('store/cart/confirm');
		$p['cancel_url'] = Yii::app()->createAbsoluteUrl('store/cart/cancel');
		$p['address'] = $data['address'];
		if(Yii::app()->user->getState('store_invoice_id'))
			$p['invoice_id'] = Yii::app()->user->getState('store_invoice_id');
		$p['email'] = $data['email'];
	...
	public static function checkCart() {
		...
		if(Yii::app()->user->getState('store_history_id')){
			echo Yii::app()->user->getState('store_history_id');
			$model = ProdHistory::model()->findByPk(Yii::app()->user->getState('store_history_id'));
			if($model !== null && $model->is_paid == 1) {
				Yii::app()->shoppingCart->clear();
				Yii::app()->user->setState('store_history_id', null);
			}
		}
	}
