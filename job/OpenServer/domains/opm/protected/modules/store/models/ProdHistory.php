...
class ProdHistory extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'invoice'=>array(self::HAS_ONE, 'StoreInvoice', 'history_id'),
		);
	}
	 ...
	 */
	public static function saveOrder($email, $orderDetails, $comment, $shipping) {
		...
		foreach($cart as $product) {
			...
			$invoiceData[] = array(
				1 =>$product['name'],
				3 => $product->getPrice(),
				2 => $product->getQuantity(),
				4=> $product->getPrice()*$product->getQuantity(),
			);
		}
		$model = null;
		if(Yii::app()->user->getState('store_history_id'))
			$model = self::model()->findByPk(Yii::app()->user->getState('store_history_id'));
		
		if($model === null || $model->is_paid == 1)
			$model = new self;
		$model->email = $email;
		...
		if($model->save()) {
			Yii::app()->user->setState('store_history_id', $model->id);
			
			Yii::import('application.modules.invoice.models.*');
			Yii::import('application.modules.invoice.components.*');
			$invoice = Invoice::model()->find('model_id=:model_id AND invoice_type = 1', array(':model_id'=>$model->id));
			if($invoice === null)
				$invoice = new Invoice;
			$invoice->name = $orderDetails['First Name'].' '.$orderDetails['Last Name'];
			$invoice->phone = $orderDetails['Phone'];
			$invoice->email = $model->email;
			$invoice->create_date = date('Y-m-d');
			$invoice->invoice_data = serialize($invoiceData);
			$invoice->invoice_type = 1;
			$invoice->model_id = $model->id;
			$invoice->status = 0;
			$invoice->shipping = $shipping;
			$invoice->tax = Settings::getVal('tax')*0.01*Yii::app()->shoppingCart->getCost();
			$invoice->address = $orderDetails['Address'];
			$invoice->total_cost = InvoiceHelper::getPayment(Yii::app()->shoppingCart->getCost(), $invoice, true);
			$invoice->save(false);
