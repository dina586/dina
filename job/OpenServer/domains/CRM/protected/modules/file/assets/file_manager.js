...
function initFileUpload(json) {
	if(json.params.filename_template && json.params.filename_template != '') {
		var re = /\{(\w+)\}/ig;
		var search = json.params.filename_template.match(re);
		//colsole.log(search);
		var fileName = json.params.filename_template;
		
		for(var i = 0; i < search.length; i++) {
			if(search[i] == '{duplicate}')
				continue;
			var item = search[i].replace('{', '').replace('}', '');
					
			json.params.filename_template = json.params.filename_template.replace(search[i], $('#'+json.params.name+'_'+item+'').val());
		}
		
	}
	return new qq.FileUploader(json);
}