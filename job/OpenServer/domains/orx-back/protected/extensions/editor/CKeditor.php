...
class CKeditor extends CInputWidget {
	...
	/**
	 * @var array URLs to extension files that should be applied to editing area.
	 */
	public $baseUrl = '';
	...
	public function init() {
		$this -> _CKEditorExtPath = dirname(__FILE__) . '/source/ckeditor';
		$this -> _publish();
	}
	...
	private function _publish() {
		if(empty(self::$_CKEditorURL)) {
			...
			$dir = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'source'. DIRECTORY_SEPARATOR . 'kcfinder';
	        $this->baseUrl = Yii::app()->getAssetManager()->publish($dir);
	    	$this->filebrowserImageBrowseUrl = $this->baseUrl."/browse.php?type=files";
	    	$this->filebrowserImageUploadUrl = $this->baseUrl."/upload.php?type=files";
		}
	}
