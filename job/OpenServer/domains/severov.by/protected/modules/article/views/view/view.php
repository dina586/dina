<?php 
	$path = Yii::getPathOfAlias('webroot').DS."upload".DS."article".DS.$model->id.".jpg";
	if(file_exists($path))
		$src = '<img src = "/upload/article/'.$model->id.'.jpg" title = "'.$model->name.'" alt = "'.$model->name.'" />';
	else
		$src = '';
?>
<?php 
$this->widget('application.components.BreadCrumb', 
    array('crumbs' => array(
             array('name' => 'Статьи', 'url' => ''.Yii::app()->createUrl('/article').''),
