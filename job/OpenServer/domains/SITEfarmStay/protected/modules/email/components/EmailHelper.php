...
class EmailHelper {
	...
	public static function generateTagsAdmin($id) {
		...
		$view = '';
		if(count($sqlData)> 0) {
			$view .= '<article class = "g-styles"><ul>';
			foreach($sqlData as $data) {
				$view .= '<li>{'.$data->email_tag->tag.'} - '.Yii::t('admin', $data->email_tag->name).' ('.Yii::app()->getModule('email')->tagType[$data->email_tag->tag_type].')</li>';
			}
	...
	public static function generateTagsDeveloper($id) {
		...
		foreach($tags as $tag) {
			...
			if($count > 0)
				...
				$v = 0;
			$view .= '<div class = "l-row l-checkbox">';
			$view .= BsHtml::checkBoxListControlGroup('EmailTags', $v, array($tag->id=>'{'.$tag->tag.'} - '.Yii::t('admin', $tag->name)));
			$view .= '</div>';
		}
